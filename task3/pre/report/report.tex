...
病例 BraTS-GLI-00124-000 提供了一个更具体的观察。图~\ref{fig:case00124_basic} 中，FLAIR 上肿瘤信号非常突出，Ground truth 包含外围水肿、肿瘤核心和增强区域的层次结构；Attention U-Net、TransBTS 和 SwinUNETR 都能定位主体肿瘤，但预测更接近单一大块区域，外围 ED 与内部 NCR/ET 的层次恢复不足。这个病例说明，三维主干能够找到病灶位置，模态信息和区域结构仍需要更细的融合机制。

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{../fig/case/BraTS-GLI-00124-000_basic_compare.png}
\caption{BraTS-GLI-00124-000 的基础模型对比。该切片中 FLAIR 信号清楚，但基础 3D 模型对 ED/NCR/ET 的区域层次恢复不足，为后续模态融合设计提供了定性动机。}
\label{fig:case00124_basic}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.72\linewidth]{../fig/case/BraTS-GLI-00124-000_msa_z065.png}
\caption{MSA-TransBTS 在 BraTS-GLI-00124-000 上的同切片结果。该图用于观察输入端模态-空间注意力对明显 FLAIR 病灶的响应。}
\end{figure}
...
测试结果与设计初衷出现明显偏差。RSF 的 Mean Dice 只有 0.8805，WT/TC/ET HD95 分别为 10.10、5.96、5.81。worst cases 中，BraTS-GLI-00321-000 的 WT/TC/ET HD95 分别达到 80.73、109.24、111.77；BraTS-GLI-00733-001 的 WT HD95 为 90.72。这类结果对应远端假阳性和不连续预测。由此得到的经验是：区域监督和病灶采样提高了敏感性，同时也放大了对疑似病灶的响应。后续 TAF 设计减少对区域辅助头的依赖，优先约束融合的数值形式。
